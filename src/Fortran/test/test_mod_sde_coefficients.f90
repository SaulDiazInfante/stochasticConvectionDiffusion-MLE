program test_mod_sde_coefficients
   ...
   ! Allocate local test arrays
    ...
    if (all(abs(vec_diff) < eps)) then
        status = .TRUE.
        print *, "compute_difussion_derivative_at_u and eval_diagonal_at_u TEST PASSED"
        print '(A10, ES30.16)', '|error|:', sum(abs(vec_diff))
        call print_vector_with_indices("compute_difussion_derivative output:", diffusion_derivative(1:10), 10)
        call print_vector_with_indices("expected (sigma * b) output:", expected_vec(1:10), 10)
        call print_vector_with_indices("error", vec_diff(1:10), 10)
    else
        print *, "compute_difussion_derivative at_u and eval_diagonal_at_u TEST FAILED"
        print '(A10, ES30.16)', '|error|:', sum(abs(vec_diff))
        call print_vector_with_indices("compute_difussion_derivative output:", diffusion_derivative(1:10), 10)
        call print_vector_with_indices("\sigma * B:", expected_vec(1:10), 10)
        call print_vector_with_indices("error", vec_diff(1:10), 10)
    end if
