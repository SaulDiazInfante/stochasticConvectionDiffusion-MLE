...
u0_proj = zeros(N2, N2);
Lambda = zeros(N2, N2);
msg = 'Reshaping matrix coefficients';
f = waitbar(0, msg);
r = 0;
for i = 1:Nx
  progress_bar(i + 1, Nx, 40, msg, f)
  for j = 1:Ny
    m = i + (j - 1) * Nx;
...
t = 0;
msg = 'Integratiing with Euler';
f = waitbar(0, msg);
for ii = 0:N_ITER - 1
    t = t + dt;
    progress_bar(ii + 1, N_ITER, 40, msg, f)
    u0_proj_row_np1 = u0_proj_row_n ...
        ;
    u0_proj_row_n = - M * u0_proj_row_n * dt + u0_proj_row_np1;
end
delete(f)
...
quiver(xgrid, ygrid, v1grid, v2grid)
contour(xgrid, ygrid, u0_comp_np1_grid,100); 
colorbar;
axis([0, Lx, 0, Ly])
